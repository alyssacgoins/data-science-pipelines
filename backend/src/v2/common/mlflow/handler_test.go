...
func TestOnTaskStart_NilPluginConfig_ReturnsEmptyString(t *testing.T) {
	handler := NewHandler()
	nestedRunID, err := handler.OnTaskStart(context.Background(), testTaskInfo_TaskStart("r1"), nil)
...
func TestOnTaskStart_MLflowFailure_ReturnsEmptyNestedRunID(t *testing.T) {
	...
	handler := NewHandler()
	nestedRunID, err := handler.OnTaskStart(context.Background(), testTaskInfo_TaskStart("r1"), testPluginConfig(server.URL))
	require.Error(t, err)
